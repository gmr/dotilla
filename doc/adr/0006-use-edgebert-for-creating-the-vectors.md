...
Storing vectors in LanceDB settles where they live. This decision is about where
they come from.
...
local but reintroduces the external process that ADR 3's single binary
...
which is a further reason to embed through an asynchronous outbox rather than
...
then a rebuild of a derived store, which the outbox makes possible.
